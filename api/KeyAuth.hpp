...
namespace KeyAuth {
	...
	public:
		...
		void log(std::string message) {
			...
			lw_http_d.add_field(PCHAR("type"), encryption::encode(XorStr("log").c_str()).c_str());
			lw_http_d.add_field(PCHAR("key"), encryption::encrypt(user_data.key, secret, iv).c_str());
			lw_http_d.add_field(PCHAR("pcuser"), encryption::encrypt(UsernamePC, secret, iv).c_str());
			lw_http_d.add_field(PCHAR("message"), encryption::encrypt(message, secret, iv).c_str());
